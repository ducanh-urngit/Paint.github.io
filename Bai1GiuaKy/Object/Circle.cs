...
namespace Bai1GiuaKy.Object
{
    ...
    {
        public Circle(Pen pen, bool isFill)
        {
            this.pen = pen;
            this.isFill = isFill;   
        }
        public Circle(Brush brush, bool isFill, Color colorBrush)
        {
            this.brush = brush;
            this.isFill = isFill;
        ...
        public override void Draw(PaintEventArgs e)
        {
            ...
            if (p2.X > p1.X) p2.X = p1.X + width;
            else p2.X = p1.X - width;
            if (p2.Y > p1.Y) p2.Y = p1.Y + width;
            else p2.Y = p1.Y - width;
            ...
            if (!isFill)
                e.Graphics.DrawEllipse(pen, start.X, start.Y, Math.Abs(p2.X - p1.X), Math.Abs(p2.Y - p1.Y));
            else
                e.Graphics.FillEllipse(brush, start.X, start.Y, Math.Abs(p2.X - p1.X), Math.Abs(p2.Y - p1.Y));
        }
        ~Circle() { }
    }
}
